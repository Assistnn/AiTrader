...
| 2026-02-25 | M4 Action Enumを実装に合わせて更新: EXIT→CLOSE, PARTIAL→PARTIAL_CLOSE, ADJUST_TRAIL→ADJUST_TP（新規）, EXTEND_TP→ADJUST_SL（新規）, trailMode→slAdjustPips | 監査承認: 実装側の命名がFX実務においてより具体的かつ柔軟（TP/SLの双方向調整対応） |
