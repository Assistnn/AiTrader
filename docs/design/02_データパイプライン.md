...
Private API（認証必要）:
  - GET /private/v1/account/assets        ... 資産残高
  - POST /private/v1/order                ... 注文
  - POST /private/v1/cancelOrders         ... 注文キャンセル（複数対応）
  - GET /private/v1/openPositions         ... 建玉一覧
  - POST /private/v1/closeOrder           ... 決済注文

WebSocket:
  - wss://forex-api.coin.z.com/ws/public/v1     ... Public
  - wss://forex-api.coin.z.com/ws/private/v1    ... Private
  - チャンネル: ticker, orderbooks, trades

KLine API仕様:
  - エンドポイント: GET /public/v1/klines
  - パラメータ: symbol, priceType(BID/ASK), interval, date
  - 対応interval: 1min, 5min, 10min, 15min, 30min, 1hour, 4hour, 8hour, 12hour, 1day, 1week, 1month
  - レスポンス: openTime, open, high, low, close（volumeなし）
...
Candlestick API仕様:
  - エンドポイント: GET /{pair}/candlestick/{candle_type}/{date}
  - 対応candle_type: 1min, 5min, 15min, 30min, 1hour, 4hour, 8hour, 12hour, 1day, 1week, 1month
  - dateフォーマット（bitbank API仕様に準拠）:
    * 1min, 5min, 15min, 30min, 1hour: YYYYMMDD形式（1日分ずつ取得）
    * 4hour, 8hour, 12hour, 1day, 1week, 1month: YYYY形式（1年分ずつ取得）
...
| 2026-03-05 | §2-2 bitbank Candlestick API日付フォーマット修正 | H4/D1等の高時間足はYYYY形式（bitbank API仕様準拠）。YYYYMMDD統一は誤記 |
| 2026-03-11 | §2-1 GMO FX APIのURL・エンドポイント修正: ベースURL→forex-api.coin.z.com、cancelOrder→cancelOrders、account/margin削除、KLineにpriceType追加・volume削除 | 外国為替FX APIは暗号通貨APIとは別ドメインで提供されており、設計書作成時に誤記載 |
