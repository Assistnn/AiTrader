...
CREATE TABLE smtp_configs (
    ...
    password_encrypted BYTEA,           -- KeyVault で暗号化
...
**notification_trigger_configs（通知トリガー設定）**
```sql
CREATE TABLE notification_trigger_configs (
    id              SERIAL PRIMARY KEY,
    user_id         INTEGER NOT NULL REFERENCES users(id),
    trigger_key     VARCHAR(50) NOT NULL,  -- notifyOnEntry, notifyOnExit, etc.
    enabled         BOOLEAN DEFAULT true,
    created_at      TIMESTAMP NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMP NOT NULL DEFAULT NOW(),
    UNIQUE(user_id, trigger_key)
);
CREATE INDEX idx_ntc_user_id ON notification_trigger_configs(user_id);
```
...
**asset_profiles（資産プロファイル - 監査対応: 設定のみで新資産種別追加）**
```sql
CREATE TABLE asset_profiles (
    id              SERIAL PRIMARY KEY,
    pair            VARCHAR(20) NOT NULL UNIQUE,
    asset_type      VARCHAR(10) NOT NULL,  -- 'fx' / 'crypto'
    pip_size        DECIMAL(10,6) NOT NULL,
    pip_digits      INTEGER NOT NULL,
    pip_value_per_lot DECIMAL(10,2) NOT NULL,
    default_min_lot DECIMAL(10,6) NOT NULL DEFAULT 0.01,
    default_max_lot DECIMAL(10,2) NOT NULL DEFAULT 100.0,
    default_tp_multiplier DECIMAL(4,2) NOT NULL DEFAULT 1.5,
    default_sl_multiplier DECIMAL(4,2) NOT NULL DEFAULT 1.0,
    created_at      TIMESTAMP NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMP NOT NULL DEFAULT NOW()
);
CREATE INDEX idx_ap_pair ON asset_profiles(pair);
CREATE INDEX idx_ap_asset_type ON asset_profiles(asset_type);
```

- `03_セーフガードエンジン.md` - safeguard_configs, safeguard_logs
- `04_判断パイプライン.md` - pipeline_logs, config_changes, model_stage_configs
- `05_AI統合.md` - ai_decision_logs, system_default_prompts
- `06_取引所抽象化.md` - exchange_configs, positions, trade_history
- `08_API仕様.md` - 全テーブルに対応するCRUDエンドポイント
- `11_セキュリティ.md` - exchange_configs/ai_model_configsの暗号化
...
| 2026-03-13 | smtp_configs, notification_trigger_configs テーブル追加 | 監査指摘: SMTP設定・通知トリガーの永続化不備 |
| 2026-03-15 | asset_profiles テーブル追加 | 監査指摘: FX/暗号通貨のpip定義を設定のみで管理。新資産種別追加時にコード変更不要を実現 |
