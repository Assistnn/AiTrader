 ...
 *
 * 制御コンポーネント: form/onChange は親から受け取る。
 * 保存は親（TraderSettingsPage）の一括保存ボタンで行う。
 */
...
export function defaultSafeguardConfig(): Record<string, unknown> {
  return {
    maxDailyLossPct: 2.0,
...
/* ================================================================
 * Main Panel (controlled component)
 * ================================================================ */
    ...
    <div className="space-y-3">
      ...
      <Category title="資金保護" codes="SG-001〜SG-009" defaultOpen>
        ...
        <NumberField
          ...
          onChange={(v) => onChange("maxMonthlyLossPct", v)}
          unit="%"
        />
        <NumberField
          label="SG-003: 最大ドローダウン"
          value={get("maxDrawdownPct", 10.0)}
          onChange={(v) => onChange("maxDrawdownPct", v)}
          unit="%"
        />
        <SwitchField
          label="SG-004: 利益達成時停止"
          checked={get("stopOnProfit.enabled", false)}
          onChange={(v) => onChange("stopOnProfit.enabled", v)}
        />
        <NumberField
          label="SG-005: 利益目標"
          value={get("stopOnProfit.targetPct", 3.0)}
          onChange={(v) => onChange("stopOnProfit.targetPct", v)}
          unit="%"
        />
        <SwitchField
          label="SG-006: 連続損失制御"
          checked={get("consecutiveLoss.enabled", true)}
          onChange={(v) => onChange("consecutiveLoss.enabled", v)}
        />
        <NumberField
          label="SG-007: 最大連続損失数"
          value={get("consecutiveLoss.maxCount", 3)}
          onChange={(v) => onChange("consecutiveLoss.maxCount", v)}
          unit="回"
        />
        <NumberField
          label="SG-008: クールダウン時間"
          value={get("consecutiveLoss.cooldownMinutes", 60)}
          onChange={(v) => onChange("consecutiveLoss.cooldownMinutes", v)}
          unit="分"
        />
        <SwitchField
          label="SG-009: ロット半減"
          checked={get("consecutiveLoss.halveLot", false)}
          onChange={(v) => onChange("consecutiveLoss.halveLot", v)}
        />
      </Category>
      ...
      <Category title="経済指標フィルター" codes="SG-027〜SG-031">
        <SwitchField
          ...
          onChange={(v) => onChange("econStop.enabled", v)}
        />
        <SelectField
          ...
          onChange={(v) => onChange("econStop.importance", v)}
        />
        <NumberField
          label="SG-029: 発表前停止時間"
          value={get("econStop.beforeMinutes", 30)}
          onChange={(v) => onChange("econStop.beforeMinutes", v)}
          unit="分"
        />
        <NumberField
          label="SG-030: 発表後停止時間"
          value={get("econStop.afterMinutes", 15)}
          onChange={(v) => onChange("econStop.afterMinutes", v)}
          unit="分"
        />
        <SelectField
          ...
          onChange={(v) => onChange("econStop.scope", v)}
        />
      </Category>
      ...
      <Category title="レジーム保護" codes="SG-032〜SG-034">
        <SwitchField
          ...
          onChange={(v) => onChange("regime.trendBreakdown", v)}
        />
        <SwitchField
          label="SG-033: ADX低下時停止"
          checked={get("regime.adxDrop", false)}
          onChange={(v) => onChange("regime.adxDrop", v)}
        />
        <SwitchField
          label="SG-034: レンジ移行時停止"
          checked={get("regime.rangeTransition", false)}
          onChange={(v) => onChange("regime.rangeTransition", v)}
        />
      </Category>
